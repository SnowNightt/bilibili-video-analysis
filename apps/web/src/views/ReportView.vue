...
const { state } = app
const failedScreenshotUrls = ref<string[]>([])

function markScreenshotFailed(url: string) {
  if (!failedScreenshotUrls.value.includes(url)) failedScreenshotUrls.value.push(url)
}

async function repairScreenshots() {
  if (await app.repairCurrentReportScreenshots()) failedScreenshotUrls.value = []
}
</script>
  ...
  <section class="page page-report">
    <template v-if="state.currentReport">
      ...
      <div class="report-layout">
        <aside class="report-toc"><strong>报告目录</strong><a href="#summary">一句话省流</a><a href="#overview">内容概览</a><a href="#chapters">章节时间线</a><a href="#points">要点梳理</a><a href="#screenshots">关键截图</a><a href="#confidence">可信度提示</a></aside>
        <article class="report-body">
          ...
          <section id="points"><h2>要点梳理</h2><ul class="point-list"><li v-for="point in state.currentReport.keyPoints" :key="point.title"><strong>{{ point.title }}</strong><p>{{ point.detail }}</p></li></ul></section>
          <section id="screenshots" v-if="state.currentReport.screenshots.length">
            <h2>关键截图</h2>
            <div class="screenshot-grid">
              <figure v-for="screenshot in state.currentReport.screenshots" :key="`${screenshot.timestampSeconds}-${screenshot.url}`">
                <div v-if="failedScreenshotUrls.includes(screenshot.url)" class="screenshot-missing">截图文件已失效</div>
                <img v-else :src="screenshot.url" :alt="screenshot.description" referrerpolicy="no-referrer" @error="markScreenshotFailed(screenshot.url)" />
                <figcaption><a :href="timestampUrl(state.currentReport.video.url, screenshot.timestampSeconds)" target="_blank">{{ formatDuration(screenshot.timestampSeconds) }}</a><span>{{ screenshot.description }}</span></figcaption>
              </figure>
            </div>
            <div v-if="failedScreenshotUrls.length" class="screenshot-repair">
              <p>部分旧报告的截图文件已被历史版本清理，可从原视频按原时间点重新提取，不会调用分析模型。</p>
              <el-button class="secondary-button" native-type="button" :loading="state.busy" :disabled="state.busy" @click="repairScreenshots">修复截图</el-button>
            </div>
          </section>
